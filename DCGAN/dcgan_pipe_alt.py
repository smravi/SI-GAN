        ...
        for epoch in range(0, n_epoch + 1):
            ...
            for step in range(n_batch_epoch):
                ...
                for _ in range(5):
                    ...
                    #                 print("ind_",ind_)
                    #                 print("sound train len",sound_train.shape)

                    idexs_w = get_random_int(min=0, max=image_train.shape[1] * len(ind) - 1, number=batch_size)

                    mis_sound = sound_train[ind].reshape(-1, sound_train.shape[2], sound_train.shape[3],
                                                         sound_train.shape[4])
                    mis_image = image_train[ind_].reshape(-1, image_train.shape[2], image_train.shape[3],
                                                          image_train.shape[4])
                    ...
                    del (mis_sound)
                    del (mis_image)
                    sound_train_flat = sound_train.reshape(-1, sound_train.shape[2], sound_train.shape[3],
                                                           sound_train.shape[4])
                    image_train_flat = image_train.reshape(-1, image_train.shape[2], image_train.shape[3],
                                                           image_train.shape[4])
                    #########################################################

                    idexs_r = get_random_int(min=0, max=image_train.shape[1] * image_train.shape[0] - 1,
                                             number=batch_size)
                    ...
                    b_real_images = threading_data(real_images, prepro_img,
                    ...
                    #                             t_z : b_z})
                    errD, _, gan_sum_string = self.sess.run([d_loss, d_optim, gan_sum], feed_dict={
                        t_real_image: b_real_images,
                        self.sound_encoder2.sound_input_placeholder: wrong_sound,
                        self.sound_encoder1.sound_input_placeholder: real_sound,
                        t_z: b_z})
