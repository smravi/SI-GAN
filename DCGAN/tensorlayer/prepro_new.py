...
def load_image(image_path):
    img_raw = scipy.misc.imread(image_path)
    img = tl.prepro.imresize(img_raw, size=[64, 64,3])
    preprocessed_img = np.asarray(img.astype(np.float32))
    return preprocessed_img


def load_sounds_from_list(name_list,load_size):
    #audios = []
    #audios = np.array(audios)
    audios = np.zeros([len(name_list),load_size,1,1])
...
def load_images_from_list(name_list, load_size):
    images = np.zeros([len(name_list), 64,64,3])
    for idx, image_path in enumerate(name_list):
        #print(image_path,"-----")
...
def preprocess(raw_audio, load_size):
    ...
    if length > raw_audio.shape[0]:
        raw_audio = np.tile(raw_audio, int(length/raw_audio.shape[0]))
    if raw_audio.shape[0] < length:
        raw_audio = librosa.util.fix_length(raw_audio,length)
    else:
...
def read_text_file(filename):
    ...
    correct_image_audios = input_file.readlines()
    random.shuffle(correct_image_audios)
    train_audio_paths_correct = []
    train_image_paths_correct = []
    train_audio_paths_mismatch = []
    train_labels = []
    for i in range(len(correct_image_audios)):
        audio_path,image_path,image_path_mismatch,label = correct_image_audios[i].split(',')
        train_audio_paths_correct.append(audio_path)
        train_image_paths_correct.append(image_path)
        train_audio_paths_mismatch.append(image_path_mismatch)
        train_labels.append(label)
    return train_audio_paths_correct,train_image_paths_correct,train_audio_paths_mismatch,train_labels
