...
class DCGAN:
    ...
    def dc_run(self):
        ...
        d_optim_adam = tf.train.AdamOptimizer(lr_v, beta1=beta1)
        d_optim_grad = d_optim_adam.compute_gradients(d_loss, var_list=d_vars)
        d_optim = d_optim_adam.apply_gradients(d_optim_grad)

        
        g_optim_adam = tf.train.AdamOptimizer(lr_v, beta1=beta1)
        g_optim_grad = g_optim_adam.compute_gradients(g_loss, var_list=g_vars)
        g_optim = g_optim_adam.apply_gradients(g_optim_grad)
        

        for d_grad in d_optim_grad:
            tf.add_to_collection('d_norms',tf.nn.l2_loss(d_grad))
        
        d_norm_sum = tf.add_n(tf.get_collection('d_norms'), name='grad_sum')
            

        # d_norm = tf.nn.l2_loss(d_optim_grad)
        # g_norm = tf.nn.l2_loss(g_optim_grad)
        ...
        for epoch in range(0, n_epoch+1):
            ...
            for step in range(n_batch_epoch):
                ...
                errD, _, gan_sum_string, d_norm_grad = \
                            self.sess.run([d_loss, d_optim, gan_sum, d_norm_sum], 
                                feed_dict={
                                            t_real_image : b_real_images,
                                            self.sound_encoder2.sound_input_placeholder: wrong_sound,
                                            self.sound_encoder1.sound_input_placeholder: real_sound,
                ...
                print("Checkpoint")
                ## updates G
                errG, _, gloss_sum_string, g_norm_grad = self.sess.run([g_loss, g_optim, gloss_sum, g_norm], feed_dict={
                                self.sound_encoder1.sound_input_placeholder : real_sound,
                                t_z : b_z})
                ...
                print("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4fs, d_loss: %.4f, g_loss: %.4f, d_grad: %.4f, g_grad: %.4f" \
