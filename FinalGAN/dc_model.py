...
def discriminator_txt2img_resnet(input_images, t_txt=None, is_train=True, reuse=False):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        # print('Image Logits:{}'.format(net_h4.outputs.shape))

        # net_h4_next = Conv2d(net_h4, 10, (s16, s16), (s16, s16), padding='VALID', W_init=w_init, name='d_h4_next/conv2d')
        # print('My Conv Layer:{}'.format(net_h4_next.outputs.get_shape()))

        # net_h5 = FlattenLayer(net_h4_next, name='d_h5/flatten')
        ...
        if t_txt is not None:
            ...
            net_h4 = BatchNormLayer(net_h4, act=lambda x: tl.act.lrelu(x, 0.2),
                    is_train=is_train, gamma_init=gamma_init, name='d_h3/batch_norm_2')
            print('Intermediate Conv Layer:{}'.format(net_h4.outputs.get_shape()))

            

        #     net_h6 = DenseLayer(net_h5, n_units=10, act=tf.identity,
        #         W_init=w_init, b_init=None, name='d_h6/dense')
        net_ho = Conv2d(net_h4, 1, (s16, s16), (s16, s16), padding='VALID', W_init=w_init, name='d_ho/conv2d')
        print('Final Conv Layer:{}'.format(net_h4.outputs.get_shape()))
        
        # Uncomment for featur loss with final laye
        ...
        # cat_logits = net_h5.outputs
        # print('Cat logits shape:{}'.format(cat_logits))
    return net_ho, logits, net_h4 #Uncomment for feature loss with intermediate layer

        # return net_ho, logits, cat_logits
